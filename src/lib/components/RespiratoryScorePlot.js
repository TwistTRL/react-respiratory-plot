...
class RespiratoryScorePlot extends PureComponent {
  ...
  getTimeSortedRSS = memoize_one( (RSS)=>{
    return RSS.sort( (a,b)=> a.time-b.time);
  });

  getTimeSortedECMOScore = memoize_one( (ECMOScore)=>{
    return ECMOScore.sort( (a,b)=> a.time-b.time);
  });

  getRSSTime = memoize_one( (RSS)=>{
    return RSS.map( ({time})=>time )
  });

  getECMOScoreTime = memoize_one( (ECMOScore)=>{
    return ECMOScore.map( ({time})=>time )
  });
  
  getRSSInRange(RSS,minX,maxX) {
    let sortedRSS = this.getTimeSortedRSS(RSS);
    let sortedTimes = this.getRSSTime(sortedRSS);
    let startIdx = bisect_left(sortedTimes,minX);
    let endIdx = bisect_right(sortedTimes,maxX);
    return RSS.slice(startIdx,endIdx);
  }

  getECMOScoreInRange(ECMOScore,minX,maxX) {
    let sortedECMOScore = this.getTimeSortedECMOScore(ECMOScore);
    let sortedTimes = this.getRSSTime(sortedECMOScore);
    let startIdx = bisect_left(sortedTimes,minX);
    let endIdx = bisect_right(sortedTimes,maxX);
    return ECMOScore.slice(startIdx,endIdx);
  }
